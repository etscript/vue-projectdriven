        ...
        <el-table 
          ...
          <el-table-column label="标签" width="200">
            <template slot-scope="scope">
              <template v-for="(item, index) in scope.row.tags_name">
          ...
          <el-table-column label="是否置顶" width="100">
            <template slot-scope="scope">
              <el-switch v-model="scope.row.top" :active-value="1" @change="topBtn(scope.row.id, scope.row.top)"></el-switch>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    getArticles() {
      this.loading = true
      // 获取软删除的数据 all=1
      this.$post('/api/article/list', this.pageModel).then(res => {
    ...
    deleteBtn(id, value) {
      ...
      if (value == true) {
        this.$post('/api/article/delete', {id}).then(res => {
    ...
    topBtn(id, value) {
      ...
      if (value == 1) {
        this.$post('/api/article/top', {id}).then(res => {
          this.$message.success(`文章${id}置顶操作成功`)
        }).catch(() => {
        })
      } else {
        this.$post('/api/article/untop', {id}).then(res => {
          this.$message.success(`文章${id}取消置顶成功`)
        }).catch(() => {
        })
      }
    }
